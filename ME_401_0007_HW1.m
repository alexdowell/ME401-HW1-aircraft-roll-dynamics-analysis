...
I_x_wingtips_n = I_x_n +(.125 *w_n_slugs)*((b_n/2)^2); % (slugs/ft^2) mass moment of inertia x when fuel is in the wingtips
...
I_x_wingtips_f = I_x_f +(.125 *w_f_slugs)*((b_f/2)^2); % (slugs/ft^2) mass moment of inertia x when fuel is in the wingtips
...
% creating state space & sep response for F-104A & Navion %
for i = [1:1:length(v)]
    sys_f(i) = ss(L_p_f, L_da_f, 1, 0);
    step_f = step(sys_f(i));
    sys_n(i) = ss(L_p_n, L_da_n, 1, 0);
    step_n = step(sys_f(i));
    % Problem 3 %
